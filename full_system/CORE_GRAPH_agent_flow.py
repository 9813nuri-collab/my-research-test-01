...
==================================================
5-Stage LangGraph 기반 실시간 시장 분석 및 포트폴리오 구성 파이프라인.

Stages
------
1. Macro Pulse Agent    — 매크로 전략 분석 & 시장 국면 판정
2. Sector/Theme Scout   — 유망 섹터 TOP 3 선정
3. Deep-Dive Micro      — 종목별 재무 데이터 수집
4. Consensus Engine     — 12인 거장 합의 시그널 도출
5. Portfolio Sizer      — 최종 자산 배분 보고서
...
# ══════════════════════════════════════════════════════════
# 2. AgentState — 에이전트 간 공유 상태 스키마
# ══════════════════════════════════════════════════════════

class AgentState(TypedDict):
    """LangGraph 5-Stage 파이프라인 공유 상태.

    모든 에이전트 노드가 읽고 쓰는 중앙 상태 스키마입니다.
    ``Annotated[List, add]`` 필드는 append 방식으로 누적됩니다.
    """

    # ── Human-in-the-Loop 입력 ──
    investment_objective: str  # 사용자 투자 목적/질문 (자연어)
    macro_context: str         # 사용자가 제공하는 현재 매크로 상황 텍스트 (선택)
    risk_tolerance: str        # "conservative" | "moderate" | "aggressive"

    # ── Stage 0: Objective Parser 출력 ──
    parsed_objective: Dict     # 파싱된 목적 구조체 (InvestmentObjective 직렬화)

    # ── Stage 1: Macro Pulse Agent 출력 ──
    market_regime: Optional[str]         # MarketRegime Enum value
    regime_confidence: float             # 국면 판정 신뢰도 (0.0~1.0)
    macro_risk_level: str                # 위험 등급 (LOW|MEDIUM|HIGH|CRITICAL)
    macro_reasoning: str                 # 판정 근거 (에이전트 간 공유용 요약)
    macro_key_factors: List[str]         # 핵심 요인 리스트 (에이전트 간 공유)
    macro_search_results: List[Dict]     # 검색 결과 원본
    live_macro_snapshot: Dict            # yfinance 수집 경제지표 스냅샷
    macro_pulse_report: str              # 사용자 친화적 매크로 분석 보고서 (사람이 읽는 용)

    # ── Stage 2: Sector Scout 출력 ──
    top_sectors: List[Dict]              # {sector_name, rationale, confidence}
    sector_analysis: str                 # 섹터 분석 보고서

    # ── Stage 3: Deep-Dive Micro 출력 ──
    target_tickers: List[str]            # 대상 종목 코드
    market_data_payloads: List[Dict]     # MarketDataPayload 호환 딕셔너리
    micro_analysis: str                  # 종목 분석 보고서

    # ── Stage 4: Consensus Engine 출력 ──
    orchestrator_outputs: List[Dict]     # OrchestratorOutput 호환 딕셔너리
    consensus_summary: str               # 거장 합의 요약

    # ── Stage 4.5: Strategic Policy Governor 출력 ──
    spg_outputs: Dict[str, Dict]        # 종목별 전략적 보정 데이터
    spg_report: str                      # 전략 에이전트 브리핑

    # ── Stage 5: Portfolio Sizer 출력 ──
    portfolio_allocation: Dict[str, float]  # 종목 → 투자 비중
    trade_plans: List[Dict]                 # 종목별 매매 계획 (진입가·목표가·손절가)
    portfolio_report: str                   # 사용자 목적에 대한 최종 답변 보고서

    # ── 시스템 제어 및 메타데이터 ──
    bypass_spg: bool                     # (레거시) SPG 우회 여부
    errors: Annotated[List[str], add]     # 에러 로그
    audit_log: Annotated[List[str], add]  # 감사 추적 로그
...
# ══════════════════════════════════════════════════════════
# 3. Brain Firmware 인지 제약 프레임워크
# ══════════════════════════════════════════════════════════

# ── 절대 인지 제약 헤더 ──
# 모든 에이전트의 System Prompt 최상단에 위치하여 LLM 자체 추론을 차단합니다.
...
def _make_firmware_prompt(agent_role_block: str, bypass_firmware: bool = False) -> str:
    """Brain Firmware를 탑재한 에이전트 시스템 프롬프트를 생성합니다.

    구조:
        [인지 제약 헤더] → [Brain Firmware 전체] → [에이전트 전용 역할/임무]

    Args:
        agent_role_block: 에이전트별 전용 역할 설명 (Firmware 하단에 부착)
        bypass_firmware: True일 경우 Firmware를 제외하고 역할 블록만 반환 (실험용)
...
# ══════════════════════════════════════════════════════════
# 4. Pydantic 출력 스키마 (LLM 구조화 출력용)
# ══════════════════════════════════════════════════════════

# MarketRegime Enum 전체 값 목록 (프롬프트 주입용)
...
class MacroPulseOutput(BaseModel):
    """Stage 1: Macro Pulse Agent의 구조화된 출력.

    LLM이 반드시 이 스키마에 맞게 출력을 생성해야 합니다.
    """
...
class InvestmentObjective(BaseModel):
    """Stage 0: 사용자 투자 목적 파싱 결과.

    자연어 투자 목적을 구조화된 필드로 변환합니다.
    """
...
# ══════════════════════════════════════════════════════════
# 5. Stage별 Brain Firmware 탑재 시스템 프롬프트 생성 함수
#    ※ 모든 에이전트는 동일한 Brain Firmware 아래에서 작동합니다.
#       각 에이전트의 역할 블록만 달라질 뿐, 인지 기반은 동일합니다.
# ══════════════════════════════════════════════════════════

def _get_stage1_system_prompt(bypass_firmware: bool = False) -> str:
    """Stage 1: Macro Pulse Agent 시스템 프롬프트 (Brain Firmware 탑재)."""
...
def _get_stage2_system_prompt(market_regime: str, bypass_firmware: bool = False) -> str:
    """Stage 2: Sector Scout Agent 시스템 프롬프트 (Brain Firmware 탑재)."""
    role_block = f"""## 레이어 주의 집중 배정
- PRIMARY LAYERS (사고력 70% 집중):
...
def _get_stage3_system_prompt(sectors: List[Dict], bypass_firmware: bool = False) -> str:
    """Stage 3: Deep-Dive Micro Agent 시스템 프롬프트 (Brain Firmware 탑재)."""
...
def _get_stage4_system_prompt(regime: str, tickers: List[str], bypass_firmware: bool = False) -> str:
    """Stage 4: Consensus Engine Node 시스템 프롬프트 (Brain Firmware 탑재)."""
...
def _get_stage5_system_prompt(
    ...
    """Stage 5: Portfolio Sizer Agent 시스템 프롬프트 (Brain Firmware 탑재).

    사용자의 투자 목적(parsed_objective)에 직접 답하는 구체적 매매 계획을 생성합니다.
    """
...
# ══════════════════════════════════════════════════════════
# 6-0. Stage 0: Objective Parser Node — 투자 목적 파서
# ══════════════════════════════════════════════════════════
...
def objective_parser_node(state: AgentState) -> Dict[str, Any]:
    ...
    사용자의 자연어 투자 목적/질문을 구조화된 InvestmentObjective로
    파싱합니다. 이 결과는 이후 모든 Stage에서 '목적 정렬(Objective Alignment)'
    기준으로 사용됩니다.
    ...
    Returns:
        상태 업데이트 딕셔너리 (parsed_objective 채움)
    """
...
# ══════════════════════════════════════════════════════════
# 6-1. Stage 1 서브 유틸: yfinance 실시간 매크로 지표 수집
# ══════════════════════════════════════════════════════════

# optimizer.py와 동일한 티커 레지스트리
...
# 임계값 참조 딕셔너리 (Brain Firmware LAYER 1 기준)
...
def _compute_r_squared(series: "pd.Series", window: int = 60) -> float:  # type: ignore[name-defined]
    """최근 window일의 선형 추세 R² 계산 (Shannon 노이즈 지표)."""
    import numpy as np
    ...
    return cc ** 2 if not (cc != cc) else 0.5  # nan 방지


def _fetch_live_macro_indicators(lookback_days: int = 300) -> Dict[str, Any]:
    """yfinance로 실시간 매크로 지표를 수집하고 LLM 주입용 요약을 생성합니다.

    optimizer.py의 fetch_macro_data / build_payload와 동일한 지표 체계를 사용하되,
    최근 상태 스냅샷(현재값 + 5일·20일 변화율 + 임계값 대비 위치)을 반환합니다.

    수집 로직:
      - lookback_days(기본 300일) 다운로드: MA200·R²(60일) 등 계산용
      - LLM에 제시하는 데이터: 최신값 + 5일·20일 변화율 (최근 20일 스냅샷)
      - 합성 지표: Graham PE, Buffett ROIC, Taleb SKEW, Soros Momentum 등

    Args:
        lookback_days: yfinance 다운로드 기간 (일수). MA200 계산을 위해 300 권장.

    Returns:
        {
            "summary_text": str,        # LLM에 주입할 포맷된 텍스트 보고서
            "current_values": Dict,     # 최신 지표 수치
            "synthetic_proxies": Dict,  # 합성 펀더멘탈 프록시 값
            "data_date": str,           # 데이터 기준 날짜
    ...
    # ── 합성 프록시 계산 (optimizer.py build_payload 로직 동일 적용) ──
...
def _generate_dynamic_search_queries(
    ...
    """투자 목적·매크로 컨텍스트·실시간 지표를 기반으로 동적 검색 쿼리를 생성합니다.

    Fix4: macro_context의 구체적 위기 키워드를 직접 추출하여
    generic 쿼리 대신 현재 상황에 특화된 쿼리를 우선 생성합니다.
    LLM 호출 없이 규칙 기반으로 생성하여 레이턴시를 최소화합니다.

    Args:
        investment_objective: 사용자 투자 목적
        macro_context:        사용자 입력 매크로 상황 텍스트
        parsed_objective:     파싱된 목적 구조체
        live_data:            yfinance 실시간 데이터 결과
    ...
    # ── Fix4: macro_context에서 구체적 위기 이벤트 직접 감지 ──
    # 중동 / 에너지 위기
    ...
    # ── 1. 위기 특화 쿼리 (macro_context 기반 최우선) ──
    if has_mideast:
        queries.append(
    ...
    # ── 2. live_data 기반 정량 위기 쿼리 (Fix4 신규) ──
...
def _search_macro_data(
    ...
    """Tavily를 사용하여 매크로 경제 데이터를 검색합니다.

    다중 쿼리로 포괄적인 매크로 정보를 수집합니다.

    Args:
        tavily:  TavilySearchResults 인스턴스
        queries: 검색 쿼리 리스트 (None이면 기본 쿼리 사용)
...
def _build_macro_context(
    ...
    """검색 결과·yfinance 지표·사용자 입력을 통합 컨텍스트로 구성합니다.

    구성 순서 (LLM 주의 집중 유도):
      1. 투자 목적    — 분석 방향 기준 (최상단)
      2. 사용자 매크로 정보 — Human-in-the-Loop 입력
      3. yfinance 경제지표  — 수치 근거 (합성 프록시 포함)
      4. 웹 검색 결과  — 최신 뉴스·트렌드

    Args:
        search_results:     Tavily 검색 결과
        investment_objective: 사용자의 투자 목적/질문 (자연어)
        macro_context:      사용자가 직접 입력한 현재 매크로 상황
        parsed_objective:   파싱된 목적 구조체
        live_macro_data:    _fetch_live_macro_indicators() 반환값
    ...
    # ── 2. 사용자 입력 매크로 정보 (Human-in-the-Loop) ──
    ...
    if search_results:
        ...
        for i, result in enumerate(search_results[:12], 1):  # 최대 12건
...
def _parse_regime_output(output: Any) -> MacroPulseOutput:
    """LLM 출력을 MacroPulseOutput으로 파싱합니다.

    구조화 출력이 이미 MacroPulseOutput이면 바로 반환하고,
    AIMessage이면 JSON 추출을 시도한 뒤, 텍스트 폴백 파싱으로 이어집니다.

    Args:
        output: LLM 응답 (MacroPulseOutput, AIMessage, 또는 str)

    Returns:
        MacroPulseOutput 인스턴스

    Raises:
        ValueError: 모든 파싱 방법 실패 시
    """
    # 이미 MacroPulseOutput이면 바로 반환
    if isinstance(output, MacroPulseOutput):
        return output

    # AIMessage에서 content 추출
    # Gemini는 content를 list[dict] 로 반환할 수 있으므로 안전하게 문자열로 변환
    raw_content = output.content if hasattr(output, "content") else str(output)
    if isinstance(raw_content, list):
        # [{"type": "text", "text": "..."}, ...] 또는 [str, ...] 형태 처리
        parts: list[str] = []
...
def _validate_regime(regime_str: str) -> MarketRegime:
    """문자열을 MarketRegime Enum으로 안전하게 변환합니다.

    정확한 값 매칭 → 대소문자 무시 매칭 → Normal_Market 폴백 순으로 시도합니다.

    Args:
        regime_str: 국면 문자열 값

    Returns:
        MarketRegime Enum 인스턴스
    """
    # 정확 매칭
    try:
...
def macro_pulse_agent(state: AgentState) -> Dict[str, Any]:
    """Stage 1: Macro Pulse Agent — 매크로 전략 분석 및 시장 국면 판정.

    4가지 데이터 소스를 통합하여 MarketRegime을 판정합니다:
      1. yfinance 실시간 경제지표 (수치 근거)
      2. 사용자 입력 매크로 정보 (Human-in-the-Loop, 높은 신뢰도)
      3. Tavily 웹 검색 (최신 뉴스·트렌드)
      4. 투자 목적 (분석 방향 기준)

    실행 흐름:
        Step 1. yfinance 경제지표 수집 (optimizer.py 동일 지표 체계)
        Step 2. 동적 Tavily 쿼리 생성 (목적·컨텍스트·지표 기반)
        Step 3. Tavily 검색 실행
        Step 4. 4소스 통합 컨텍스트 구성
        Step 5. Brain Firmware 탑재 Gemini 국면 판정
        Step 6. MarketRegime Enum 검증
        Step 7. 긴급 리스크 경고 판단
    ...
    macro_context_input  = state.get("macro_context", "")    # 사용자 입력 매크로
    parsed_objective     = state.get("parsed_objective", {})

    # NO_REGIME variant: 국면 판정을 건너뛰고 Normal_Market으로 고정
    ...
    try:
        ...
        llm = _init_gemini_llm(temperature=0.0)   # Fix2: regime 판정 일관성 확보
        ...
        if not live_data.get("fetch_error"):
            ...
            # Fix3: 보고서 스코어카드에 반드시 포함하도록 지시 추가 (human_content에 주입용)
            _trend_signals_note = (
        ...
        # 단일 llm.invoke() — 보고서 + JSON 동시 수신 (with_structured_output 미사용)
        response = llm.invoke([
        ...
        if json_section_match:
            ...
            # JSON_END 없이 JSON_START만 있을 경우 (끝까지 잘린 경우)
        ...
        if report_match:
            ...
            # REPORT_END 없이 잘린 경우: REPORT_START 이후 끝까지 저장
            ...
            if report_start_match:
                ...
                # 보고서 섹션이 전혀 없는 경우: JSON 이후 텍스트를 보고서로 사용
...
def sector_scout_agent(state: AgentState) -> Dict[str, Any]:
    """Stage 2: Sector/Theme Scout — 유망 섹터 TOP 3 선정.

    Stage 1의 Market Regime을 바탕으로 Tavily 데이터를 교차 검증하여 유망 섹터를 선정합니다.

    Brain Firmware 주입:
        _get_stage2_system_prompt(regime, strategy) 가 LLM 호출 시 사용됩니다.
        Dalio Global_Weight_Adjuster → Fisher/Lynch/Buffett 렌즈 순으로 섹터 필터링.
    ...
    try:
        ...
        # ── STEP 5: Gemini 섹터 선정 (Brain Firmware 주입) ──
        ...
        # candidate tickers 목록 취합. (종목 수는 일단 제시된 것만)
        target_tickers = []
...
def _fetch_kr_fundamentals(raw_ticker: str) -> Dict[str, float]:
    """네이버 금융을 크롤링하여 한국 주식의 핵심 재무 지표를 수집합니다.

    수집 지표: PER, PBR, ROE(프록시), EPS, BPS, 부채비율, 영업이익률,
    매출액, 영업이익, 배당수익률, 현재가.
    """
    import requests
...
def _fetch_micro_data(ticker: str) -> Dict[str, float]:
    """yfinance를 사용하여 개별 종목의 마이크로 재무 데이터를 수집합니다.

    12인 거장 Intelligence_Structure Step 1 metrics에 필요한
    핵심 지표들을 수집/계산/매핑합니다.
    (yfinance 데이터 누락 시 네이버 금융 데이터로 보완합니다.)
    """
    import yfinance as yf
    import pandas as pd
    import numpy as np
    ...
    try:
        ...
        # EPS Growth (TTM) — yfinance 우선, Naver EPS + 현재가로 추정
        ...
        if epsg_yf:
            ...
            # E/P ratio로 성장성 프록시 역산
            metrics["EPS_Growth_TTM"] = nv_data["EPS_VAL"] / nv_data["CURRENT_PRICE"]
        else:
            metrics["EPS_Growth_TTM"] = 0.0

        # ──── 모멘텀/리스크 지표 (Soros, Simons, Shannon, Thorp) ────

        # Price_Momentum_1yr_Percentile (1년 수익률 백분위 추정)
        ...
        # Price_Mean_Reversion_Zscore (20일 이평선 Z-Score)
        ...
        metrics["Order_Book_Imbalance_Ratio"] = 0.5   # 호가창 데이터 부재

        # Price_Trend_R_Squared (최근 60일 R^2)
        ...
        # Thorp Parameters (기본값 세팅, Orchestrator/Engine에서 보정)
        metrics["Expected_Value_of_Signal"] = 0.1
        metrics["Historical_Win_Rate_of_Signal"] = 0.6

        metrics["Yield_Curve_Spread_10Y_2Y"] = 0.0 # MacroPulse에서 Global 처리
...
def deep_dive_micro_agent(state: AgentState) -> Dict[str, Any]:
    """Stage 3: Deep-Dive Micro Agent — 종목별 재무 데이터 수집.

    선정된 섹터 내 종목들의 실제 재무 데이터(yfinance)를 수집하고
    MarketDataPayload 규격에 맞는 데이터를 생성합니다.
    ...
    try:
        ...
        # Brain Firmware 탑재 시스템 프롬프트 준비 (LLM 연동 시 사용될 보고서 작성용)
        # Stage 3는 직접적 LLM 호출보다는 수집/변환에 목적이 큼.
        bypass_fw = state.get("_variant_no_firmware", False)
...
def consensus_engine_node(state: AgentState) -> Dict[str, Any]:
    """Stage 4: Consensus Engine — 12인 거장 합의 시그널 도출.

    Stage 3의 MarketDataPayload를
    engine_core.py의 GraphOrchestrator에 주입하여
    12인 거장의 합의된 시그널을 도출합니다.
    ...
    try:
        ...
        # NO_RISK: Taleb, Shannon, Thorp 노드 제거
        ...
        for p_dict in payloads_dict_list:
            try:
                ...
                # 시그널 해석 (state를 extra_context로 전달하여 엔진 수준 변종 처리 지원)
                # NO_REGIME variant: 엔진 수준에서 국면 제약을 무시하도록 extra_context를 통해 전달
                # engine_core.GraphOrchestrator.resolve_signals가 이를 지원하는지 확인 필요
                output = orchestrator.resolve_signals(payload, dummy_nlp, extra_context=state)

                # FLAT_ENS variant: 단순 평균으로 앙상블 시그널 대체
...
def strategic_policy_governor(state: AgentState) -> Dict[str, Any]:
    """Stage 4.5: Strategic Policy Governor — 퀀트 신호와 투자 전략의 조화.
    
    12인 거장 퀀트 엔진(Stage 4)의 결과에 사용자의 투자 목적(DCA, 장기투자 등)을
    반영하여 전략적 하한선(Strategic Floor)을 설정하는 '투자 위원회 의장' 역할을 수행합니다.
    """
    ts = datetime.now(timezone.utc).isoformat()
    logger.info("=" * 60)
    ...
    # ── [실험용] SPG 비활성화 로직 (Ablation Study) ──
    ...
    # ── Module 3: Regime Attenuation (상향 제한 계수) ──
    regime_multipliers = {
        "Normal_Market": 1.0,
        "High_Volatility": 0.7,
        "Bear_Market": 0.5,
    ...
    # ── Module 4: Horizon Harmonizer (기간 보정 계수) ──
    ...
    # ── Module 1: Strategy Resonator (기본 하한선) ──
    base_floors = {
    ...
    for out in orchestrator_outputs:
        ...
        # ── Module 2: Contrarian Opportunity (역발상 보너스) ──
        ...
        if metrics.get("Price_Momentum_1yr_Percentile", 0) > 0.1: q_count += 1 # 하위 10% 제외
        ...
        adj_floor = min(0.20, adj_floor) # 종목당 최대 20% 제한
...
def portfolio_sizer_agent(state: AgentState) -> Dict[str, Any]:
    """Stage 5: Portfolio Sizer — 사용자 투자 목적에 대한 직접 답변 생성.

    사용자의 투자 목적(parsed_objective)을 최우선으로 반영하여
    종목별 진입가·목표가·손절가·매수량을 포함한 구체적 매매 계획을 생성합니다.

    Thorp Kelly + Shannon 필터 + Taleb 꼬리 위험 보정으로
    포지션 사이징을 결정하고 투자자 성향을 최종 조정합니다.

    Args:
        state: 현재 AgentState

    Returns:
        상태 업데이트 딕셔너리 (trade_plans + portfolio_allocation + portfolio_report)
    """
    ts = datetime.now(timezone.utc).isoformat()
    logger.info("=" * 60)
    ...
    # Stage 4 결과에서 ensemble_signal 평균 추출
    orchestrator_outputs = state.get("orchestrator_outputs", [])
    ...
    try:
        ...
        # 간단한 portfolio_allocation 추출 시도 (JSON 파싱)
        import json
        import re
        ...
        return {
            ...
            "trade_plans": [],   # 추후 TradePlanItem 파싱 고도화 시 추가
...
def route_after_macro_pulse(
    ...
    """Stage 1 이후 라우팅: 긴급 리스크면 즉시 종료, 아니면 계속.

    긴급 국면(Market_Crash, Systemic_Crash 등) 감지 시
    후속 Stage를 건너뛰고 즉시 긴급 보고서를 생성합니다.

    Args:
        state: 현재 AgentState

    Returns:
        "proceed" 또는 "critical_risk"
    """
    regime = state.get("market_regime", "Normal_Market")
...
def emergency_shutdown_node(state: AgentState) -> Dict[str, Any]:
    ...
    치명적 시스템 리스크 감지 시 실행됩니다.
    모든 투자 활동을 중단하고 긴급 보고서를 생성합니다.
    Taleb 블랙스완 프로토콜에 준하는 즉각 대응입니다.
...
def stage3_mock_node(state: AgentState) -> Dict[str, Any]:
    """[실험용] Stage 3 micro-analysis 결과를 강제로 주입함. 
    Master JSON 지표명과 100% 일치시켜 Stage 4 신호 발생을 보장함.
    """
    ...
    payloads = [
        {
            "ticker": "005930.KS", # 삼성전자 (우량 가치주 시뮬레이션)
            "metrics": {
        ...
        {
            "ticker": "051910.KS", # LG화학 (고성장/고위험 부실주 시뮬레이션)
            "metrics": {
        ...
        {
            "ticker": "RISK_STOCK", # 위험 종목 (신호는 매우 좋으나 펀더멘탈 부실)
            "metrics": {
                ...
                "Expected_Value_of_Signal": 0.95, # 퀀트 신호는 최상 (함정)
...
def build_graph():
    """H-PIOS v8.5 Field Command 5-Stage 파이프라인 그래프를 구성합니다."""
    ...
    # ── 엣지 연결 ──
    workflow.add_edge(START, "stage3_mock") # START에서 바로 데이터 주입으로 시작 (실험 속도 향상)
...
# ══════════════════════════════════════════════════════════
# 12. Module-level Graph Instance (LangGraph Studio 엔트리포인트)
# ══════════════════════════════════════════════════════════

# LangGraph Studio / langgraph dev 가 이 변수를 자동으로 탐지합니다.
# langgraph.json 의 "graphs": { "agent": "./CORE_GRAPH_agent_flow.py:app" } 와 대응됩니다.
app = build_graph()
...
def run_pipeline(
    ...
    """H-PIOS v8.5 Field Command 파이프라인을 실행합니다.

    Args:
        investment_objective: 사용자의 투자 목적/질문 (자연어).
            예) "1년 이상 장투할건데 코스피 유망 종목을 얼마일때 몇주 사서
                 언제 익절/손절할까?"
        macro_context: 사용자가 직접 입력하는 현재 매크로 상황 텍스트 (선택).
...
if __name__ == "__main__":
    ...
    # 하위 호환성: --briefing/-b 도 investment_objective로 처리
    parser.add_argument(
        "--briefing",
        "-b",
        type=str,
    ...
    # --objective 우선, 없으면 --briefing 하위 호환
